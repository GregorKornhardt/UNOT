...
def parse_args():
    ...
    parser.add_argument('--width_generator', type=int, default=4, help='Width of generator network, as a multiple of the length of the data')
    ...
    parser.add_argument('--gamma_generator', type=float, default=0.99996, help='Gamma for learning rate scheduler for generator')
    ...
    parser.add_argument('--lr_gen', type=float, default=0.01, help='Learning rate')
    parser.add_argument('--sobel', type=bool, default=False)
    parser.add_argument('--grid', type=bool, default=False)
    parser.add_argument('--grf', type=bool, default=True)
    parser.add_argument('--data_28_64', type=bool, default=True)
    parser.add_argument('--random_shapes', type=bool, default=False)
    parser.add_argument('--numbr_latend_samples', type=int, default=100)
    parser.add_argument('--generator', type=str, default='MLP')
    parser.add_argument('--lr_pred', type=float, default=1e-4, help='Learning rate')
    parser.add_argument('--use_data', type=bool, default=False, help='If you also additionally want to use data in training')
...
def main():
    ...
    if args.generator == 'MLP':
        generator = Generator(dim_prior=dimesion_latent, dim=dimension, dim_hidden=dim_generator, 
                                       num_layers=args.num_layers, dust_const=args.dust_const,
                                       skip_const=1).to(device)
    else:
        generator = FNO2d(2,2,(args.modes_x, args.modes_y), args.fno_width, 
