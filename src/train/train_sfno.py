...
class Training():
    def __init__(self,
        ...
        self.cost_matrix = cost.get_cost_matrix_S2(self.length, device=self.device)
        ...
        if adam_neuralop:
            ...
            self.predictor_optimizer = AdamW(predictor.parameters(), lr=self.lr_fno, 
    ...
    def train(self):
        data_loader = [torch.utils.data.DataLoader(data, batch_size=self.batch_size, shuffle=True) for data in self.data_set]

        # initializing learning rate schedulers

        generator_scheduler = torch.optim.lr_scheduler.ExponentialLR(self.generator_optimizer, 
                                                                     gamma=self.multiplicative_factor_generator)
        predictor_scheduler = torch.optim.lr_scheduler.ExponentialLR(self.predictor_optimizer, 
        ...
        for i in tqdm(range(self.numbr_training_iterations)):
            ...
            if self.use_data:
                ...
                data_loaders = data_loader + [data_loader_pred]
                for datas in zip(*data_loaders):
                    for data in datas[-1]:
                        self.mini_loop_predictor_data(data[:,0], data[:,1], data[:,2])
                        print(self.train_losses['predictor_data'][-1])
                        wandb.log({'predictor_loss_data': self.train_losses['predictor_data'][-1]})
                    self.mini_loop_predictor(datas[-1][:,0], datas[-1][:,1])   
                    wandb.log({'predictor_loss_gen': self.train_losses['predictor_gen'][-1]})
            else:
                for data in data_loader_pred:
                    dim = torch.randint(high=64-20, size=(1,)) +20
                    dim = dim[0]
    ...
    def mini_loop_predictor(self, mu, nu, dim):
        ...
        cost_matrix = cost.get_cost_matrix_S2_S2(dim, device=self.device)
        # calculate the target from the sinkhorn algorithm
        g_true, nan_mask = self.target_sinkhorn(mu, nu, cost_matrix)# self.target_sinkhorn(mu, nu)
        ...
        predictor_loss = torch.mean((g_predicted - g_calculated)**2)#hilbert_projoection(g_predicted, g_calculated)
    ...
    def mini_loop_gen(self, z):
        ...
        # compute the loss
        g_predicted = self.apply_predictor(mu_no_nan, nu_no_nan)
        generator_loss =  - torch.mean((g_predicted - g_calculated)**2)#hilbert_projoection(g_predicted, g_calculated)
