...
class Training:
    def __init__(
        ...
        self.generator_scheduler = torch.optim.lr_scheduler.ExponentialLR(
            self.generator_optimizer, gamma=self.multiplicative_factor_generator
        )
        self.predictor_scheduler = torch.optim.lr_scheduler.ExponentialLR(
            self.predictor_optimizer, gamma=self.multiplicative_factor_predictor
        )
    ...
    def train(self):
        for i in tqdm(range(self.numbr_training_iterations)):
            ...
            data_gen = torch.stack((mu, nu, eps.repeat(1, nu.shape[-1])), 1).detach()

            data_loader_pred = torch.utils.data.DataLoader(
                data_gen, batch_size=self.batch_size, shuffle=False
            )
            data_loader_gen = torch.utils.data.DataLoader(
                z, batch_size=self.batch_size, shuffle=True
            )
            ...
            if self.use_data:
                ...
                for data in data_loader_pred:
                    dim = torch.randint(high=self.length - 10, size=(1,)) + 10
                    dim = dim[0]
                    self.mini_loop_predictor(data[:, 0], data[:, 1], data[:, 2], dim)

            for data in data_loader_gen:
                self.mini_loop_gen(data)
    ...
    def mini_loop_predictor(self, mu, nu, eps, dim):
        ...
        # compute the loss
        g_predicted = self.apply_predictor(mu_no_nan, nu_no_nan, eps_non_nan)
    ...
    def mini_loop_gen(self, z):
        ...
        g_true, nan_mask = self.target_sinkhorn(mu, nu)
        ...
        # compute the loss
        g_predicted = self.apply_predictor(
            mu_no_nan, nu_no_nan, 0.01 * torch.ones_like(mu_no_nan)
        )
    ...
    def latent_samples(self):
        if self.which_gen == "FNO":
            ...
            z = torch.randn(
                self.numbr_latent_samples, 2 * self.dim_prior, device=self.device
            )

        epsilon = torch.exp(
            torch.rand(self.numbr_latent_samples, 1, device=self.device)
            * (torch.log(torch.tensor(2.0)) - torch.log(torch.tensor(0.01)))
    ...
    def convert_image(self, img: torch.Tensor, k: int) -> torch.Tensor:
        ...
        img_int = F.interpolate(
            img, size=(k, k), mode="bilinear", align_corners=False
        ).reshape(-1, k**2)
        # renormalize
    ...
    def save_measure(self):
        with torch.no_grad():
            ...
            self.old_fix_mu.append(
                mu.detach().cpu().reshape(-1, self.length, self.length)
            )
            self.old_fix_nu.append(
                nu.detach().cpu().reshape(-1, self.length, self.length)
            )
    ...
    def plot(self):
        ...
        plot_measures.plot_complete_measures(
            self.old_fix_mu, self.old_fix_nu, self.name + "fix", path
        )
        ...
        plot_measures.plot_measure_tight(
            self.old_fix_mu, self.old_fix_nu, self.name + "fix", path
        )
        plot_measures.plot_measure_short(self.old_mu, self.old_nu, self.name, path)
        plot_measures.plot_measure_short(
            self.old_fix_mu, self.old_fix_nu, self.name + "fix", path
        )
        measure_dict = {
            "mu": self.old_mu,
            "nu": self.old_nu,
            "mu_fix": self.old_fix_mu,
            "nu_fix": self.old_fix_nu,
        }
        ...
        if self.sobel:
            ...
            input = torch.cat(
                (
                    ...
                    grid.repeat(mu.shape[0], 1, 1, 1),
                    mu_s,
                    nu_s,
                    eps.reshape(-1, 1, length, length),
                ),
                1,
            ).reshape(-1, 9, length, length)
    ...
    def target_sinkhorn(self, mu, nu):
        with torch.no_grad():
            ...
            _, v = sinkhorn.sink_vec(
                mu,
                nu,
                self.get_cost_matrix(mu.shape[1]),
                0.01,
                nu0,
    ...
    def target_sinkhorn_var_eps(self, mu, nu, eps, cost_matrix):
        with torch.no_grad():
            # Bootstrap the empirical measures
            nu0 = torch.exp(self.apply_predictor(mu, nu, eps))
            _, v = sinkhorn.sink_vec_eps(
                mu, nu, cost_matrix, eps[:, 0], nu0, self.sinkhorn_max_iterations
            )
    ...
    def checkpoint(self, i):
        if i % 100 == 0:
            print(
                f'Generator 64 loss: {self.train_losses["generator 64"][-1]}',
                f'Predictor Gen loss: {self.train_losses["predictor_gen"][-1]}',
            )
            self.test_performance()
        if i % (self.numbr_training_iterations // 10) == 0:
            self.save_measure()
            print(f"{self.path}/Models/fno_model_save_{self.name}_{i}.pt")
            torch.save(
                self.predictor.state_dict(),
                f"{self.path}/Models/fno_model_save_{self.name}_{i}.pt",
            )
    ...
    def test_performance(self):
        ...
        def test_loss(test_set, cost_matrix, length):
            ...
            test_dist = []
            self.predictor.eval()
            eps = torch.ones(mu.shape[0], 1, device=self.device) * 0.01
            nu0 = torch.exp(self.apply_predictor(mu, nu, eps.repeat(1, nu.shape[-1])))
            with torch.no_grad():
                _, _, test_dist = sinkhorn.sink_vec_dist(
                    mu, nu, cost_matrix, self.sinkhorn_epsilon, nu0, 1
                )
            test_dist = test_dist.cpu()
            cond = ~torch.isnan(test_dist)
            relative_error = (
                torch.abs(test_dist[cond] - true_dist[cond]) / true_dist[cond]
            ).mean()
