...
class Training():
    ...
    def mini_loop_gen(
        ...
        # compute the loss
        g_predicted = self.apply_predictor(mu_no_nan, nu_no_nan, 0.01 * torch.ones_like(mu_no_nan))
    ...
    def apply_predictor(
        ...
        if self.sobel:
            ...
            input = torch.cat((mu.reshape(-1,1,length,length), nu.reshape(-1,1,length,length), grid.repeat(mu.shape[0], 1,1,1), mu_s, nu_s, eps.reshape(-1,1,length,length)), 1).reshape(-1, 9, length, length)
        
        elif self.grid:
            grid = self.get_grid(length)
            input = torch.cat((mu.reshape(-1,1,length,length), nu.reshape(-1,1,length,length), grid.repeat(mu.shape[0], 1,1,1), eps.reshape(-1,1,length,length) ), 1).reshape(-1, 5, length, length)
        
        else:
            input = torch.cat((mu.reshape(-1,1,length,length), nu.reshape(-1,1,length,length), eps.reshape(-1,1,length,length)), 1).reshape(-1, 3, length, length)
    ...
    def target_sinkhorn(
        ...
        with torch.no_grad():
            ...
            _, v = sinkhorn.sink_vec(mu, nu, self.get_cost_matrix(mu.shape[1]), 0.01, nu0, self.sinkhorn_max_iterations)
    ...
    def target_sinkhorn_var_eps(
        ...
        with torch.no_grad():
            # Bootstrap the empirical measures
            nu0 = torch.exp(self.apply_predictor(mu, nu, eps))
            _, v = sinkhorn.sink_vec_eps(mu, nu, cost_matrix, eps[:,0], nu0, self.sinkhorn_max_iterations)
