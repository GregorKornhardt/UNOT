...
class Training:
    ...
    def train(self):
        for i in tqdm(range(self.numbr_training_iterations)):
            for _ in range(self.numbr_mini_loop_predictor):
                ...
                wandb.log(
                    {
                        "generator_loss": self.train_losses["generator"][-1],
            ...
            if i % 10 == 0:
                print(
                    f'Generator loss: {self.train_losses["generator"][-1]}',
                    f'Predictor loss: {self.train_losses["predictor"][-1]}',
                )
    ...
    def mini_loop(
        ...
        epsilon_rand = torch.exp(
            torch.rand(self.numbr_rand_sample - 1, 1, device=self.device)
            * (torch.log(torch.tensor(10.0)) - torch.log(torch.tensor(0.01)))
        ...
        mu_no_nan, nu_no_nan, epsilon_no_nan = (
            mu[nan_mask],
            nu[nan_mask],
            epsilon[nan_mask],
        )
        ...
        # compute the loss
        g_predicted = self.predictor(mu_no_nan, nu_no_nan, epsilon_no_nan).reshape(
            -1, self.length**2
        )
    ...
    def target_sinkhorn(self, mu, nu, epsilon):
        with torch.no_grad():
            ...
            _, v = sink_vec_eps(
                mu, nu, self.cost_matrix, epsilon, nu0, self.sinkhorn_max_iterations
            )
    ...
    def target_sinkhorn_rand(self, mu, nu, epsilon=0.01):
        ...
        with torch.no_grad():
            for i in range(self.numbr_rand_sample):
                nu0 = torch.exp(
                    self.predictor(
                        mu[i * nbr_batch : (i + 1) * nbr_batch],
                        nu[i * nbr_batch : (i + 1) * nbr_batch],
                        epsilon[i * nbr_batch : (i + 1) * nbr_batch],
                    ).reshape(-1, self.length**2)
                )
                u_, v_ = sink_vec(
                    mu[i * nbr_batch : (i + 1) * nbr_batch],
                    nu[i * nbr_batch : (i + 1) * nbr_batch],
    ...
    def test_performance(self):
        ...
        if self.length == 64:
            ...
            def test_loss(test_set, eps):
                ...
                nu0 = torch.exp(
                    self.predictor(
                        mu, nu, torch.ones(mu.shape[0], 1, device=self.device) * eps
                    )
                )
                with torch.no_grad():
                    _, _, dist1 = sink_vec_dist(
                        mu, nu, self.cost_matrix, self.sinkhorn_epsilon, nu0, 1
                    )
            ...
            for key in self.test_data.keys():
                for key2 in self.test_data[key].keys():
                    ...
                    if key == "0.01":
                        ...
                        wandb.log(
                            {"Relative error " + key2 + " " + key: test_data_distance}
                        )
            ...
            def test_loss(test_set):
                ...
                nu0 = torch.exp(
                    self.predictor(
                        mu, nu, torch.ones(mu.shape[0], 1, device=self.device) * 0.01
                    )
                )
                with torch.no_grad():
                    _, _, dist1 = sink_vec_dist(
