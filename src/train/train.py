    ...
    def apply_predictor(self, mu, nu):
        ...
        if self.sobel:
            ...
            input = torch.cat(
                ...
                1,
            ).reshape(-1, 4, length, length)
        else:
            input = torch.cat(
                (mu.reshape(-1, 1, length, length), nu.reshape(-1, 1, length, length)),
                1,
            ).reshape(-1, 2, length, length)
