...
class SpectralConv2d(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, modes, n_layers: int = 4, hidden_width=4
    ):
        ...
        self.layers = nn.ModuleList(
            [
                Complex_Linear_Layer(
                    in_channels, hidden_width * out_channels, modes[0], modes[1]
                )
            ]
        )
        self.layers.extend(
    ...
    def forward(self, x):
        ...
        x_hat_under_modes = x_hat[
            :,
            :,
            : self.num_modes_x(spatial_points_x),
            : self.num_modes_y(spatial_points_y),
        ]

        if (
            x_hat_under_modes.shape[-2] < self.modes[0]
            or x_hat_under_modes.shape[-1] < self.modes[1]
        ):
            x_hat_under_modes = self.zero_padding_modes(
                x_hat_under_modes, self.modes[0], self.modes[1]
            )

        out_hat_under_modes = self.network(x_hat_under_modes)

        out_hat = self.zero_padding_fft(
            out_hat_under_modes, spatial_points_x, spatial_points_y
        )

        out = torch.fft.irfft2(out_hat, s=(spatial_points_x, spatial_points_y))
        return out.view(
            batch_size, self.out_channels, spatial_points_x, spatial_points_y
        )
...
class Complex_Linear_Layer(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, modes_x, modes_y, bias: bool = True):
        ...
        self.real_weights = nn.Parameter(
            scale * torch.rand(in_channels, out_channels, modes_x, modes_y)
        )
        self.imag_weights = nn.Parameter(
            scale * torch.rand(in_channels, out_channels, modes_x, modes_y)
        )
        if bias:
            self.real_bias = nn.Parameter(
                scale * torch.rand(out_channels, modes_x, modes_y)
            )
            self.imag_bias = nn.Parameter(
                scale * torch.rand(out_channels, modes_x, modes_y)
            )
        else:
    ...
    def complex_mult2d(self, x_hat, w):
        ...
        real = torch.einsum(
            "b iMN, i o MN -> b o MN", x_hat.real, w.real
        ) - torch.einsum("b iMN, i o MN -> b o MN", x_hat.imag, w.imag)
        imag = torch.einsum(
            "b iMN, i o MN -> b o MN", x_hat.real, w.imag
        ) + torch.einsum("b iMN, i o MN -> b o MN", x_hat.imag, w.real)
