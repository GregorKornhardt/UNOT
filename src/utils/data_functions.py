...
def rand_shapes(n_samples : int, dim : int, dust_const : float,
    ...
    for i in range(n_samples):
        image1 = random_shapes((length, length), max_shapes=20, min_shapes=2,
...
def get_chinese_mnist(n_samples : int, path : str) -> None:
    ...
    dataset = torchvision.datasets.ImageFolder(root=path, transform=torchvision.transforms.Grayscale())
    transformer = torchvision.transforms.ToTensor()
...
def random_set_measures(first_measure : str,
                 second_measure : str,
                 number_of_samples : int,
                 length : int = 28,
                 dust_const : int = 1e-4) -> (torch.Tensor, torch.Tensor):
    '''
    ...
    random_indices_first = np.random.choice(first_measure.shape[0], number_of_samples, replace=True)
    random_indices_second = np.random.choice(second_measure.shape[0], number_of_samples, replace=True)
    
    return torch.tensor(first_measure[random_indices_first]), torch.tensor(second_measure[random_indices_second])


def create_test_set(num_elements, length, dust_const, epsilon, device):
    with torch.no_grad():
    ...
    def distance(mu, nu):
            ...
            for i in tqdm(range(mu.shape[0])):
                _,_,_,dist1000 = sinkhorn.sink(mu[i], nu[i], cost_matrix, epsilon, torch.ones_like(mu[0]), 1000)
    ...
    dist_mnist = {'mu': mnist[0:num_elements].to('cpu'), 'nu': mnist[1:num_elements + 1].to('cpu'), 'dist': test_set_mnist}
    dist_lfw = {'mu': lfw[0:num_elements].to('cpu'), 'nu': lfw[1:num_elements + 1].to('cpu'), 'dist': test_set_lfw}
    dist_cifar = {'mu': cifar[0:num_elements].to('cpu'), 'nu': cifar[1:num_elements + 1].to('cpu'), 'dist': test_set_cifar}
    dist_bear = {'mu': bear[0:num_elements].to('cpu'), 'nu': bear[1:num_elements + 1].to('cpu'), 'dist': test_set_bear}
    dist_bear_lfw = {'mu': bear[0:num_elements].to('cpu'), 'nu': lfw[1:num_elements + 1].to('cpu'), 'dist': test_set_bear_lfw}
    dist_mnist_cifar = {'mu': mnist[0:num_elements].to('cpu'), 'nu': cifar[1:num_elements + 1].to('cpu'), 'dist': test_set_mnist_cifar}

    test_set = {'mnist': dist_mnist, 'lfw': dist_lfw, 'cifar': dist_cifar, 'bear': dist_bear, \
                'bear_lfw': dist_bear_lfw, 'mnist_cifar': dist_mnist_cifar}
    
    torch.save(test_set, f'Data/test_set__dim_{length}__eps_{epsilon}.pt')


def create_data_set(num_elements, length, dust_const, epsilon, number_of_samples, device):
    with torch.no_grad():
        mnist = torch.load('Data/mnist.pt', weights_only=True)
        lfw = torch.load('Data/lfw.pt', weights_only=True)
        cifar = torch.load('Data/cifar.pt', weights_only=True)
    ...
    def get_g(mu, nu, n):
        ...
        _, v = sinkhorn.sink_vec(mu[rand_indices1], nu[rand_indices2], cost_matrix, epsilon, torch.ones_like(mu[rand_indices1]), 50)
        
        data = torch.stack((mu[rand_indices1], nu[rand_indices2], torch.log(v)), dim=1).to('cpu')
    ...
    print('MNIST')
    test_set_mnist = get_g(mnist[0:num_elements], mnist[1:num_elements + 1], number_of_samples)
    print('LFW')
    test_set_lfw = get_g(lfw[0:num_elements], lfw[1:num_elements + 1], number_of_samples)
    print('CIFAR')
    test_set_cifar = get_g(cifar[0:num_elements], cifar[1:num_elements + 1], number_of_samples)
    print('Bear')
    test_set_bear = get_g(bear[0:num_elements], bear[1:num_elements + 1], number_of_samples)
    print('Bear LFW')
    test_set_bear_lfw = get_g(bear[0:num_elements], lfw[1:num_elements + 1], number_of_samples)
    print('MNIST CIFAR')
    test_set_mnist_cifar = get_g(mnist[0:num_elements], cifar[1:num_elements + 1], number_of_samples)
    print(test_set_mnist.shape)
    test_set = torch.cat((test_set_mnist, test_set_lfw, test_set_cifar, test_set_bear, 
                          test_set_bear_lfw, test_set_mnist_cifar), dim=0)
    print(test_set.shape)
    torch.save(test_set, f'Data/data_set__dim_{length}__eps_{epsilon}.pt')


def create_data_set_grf(num_samples, num_elements, length, dust_const, epsilon, 
                        sinkhorn_iter, incl_random_shapes: bool = False, sampling=False, device: str = 'cpu'):
    cost_matrix = cost.get_cost_matrix(length, device)
    def get_g_rand(mu, nu, n):
            rand_indices1 = torch.randint(0, mu.shape[0], (n,))
            rand_indices2 = torch.randint(0, mu.shape[0], (n,))
            _,v = sinkhorn.sink_vec(mu[rand_indices1], nu[rand_indices2], 
                                    cost_matrix, epsilon, torch.ones_like(mu[rand_indices1]), sinkhorn_iter)
    ...
    def get_g(mu, nu):
            _, v = sinkhorn.sink_vec(mu, nu,cost_matrix, epsilon, torch.ones_like(mu), sinkhorn_iter)
    ...
    for i in range(num_samples//2):
        mu = grf.gaussian_random_field(alpha =5,size=length, flag_normalize=False).to(device)
        nu = grf.gaussian_random_field(alpha =5,size=length, flag_normalize=False).to(device)
        Mu.append(mu)
        Mu.append(mu**2)
        Nu.append(nu)
        Nu.append(nu**2)
    for i in range(num_samples//2):
        mu = grf.gaussian_random_field(alpha =3,size=length, flag_normalize=False).to(device)
        nu = grf.gaussian_random_field(alpha =3,size=length, flag_normalize=False).to(device)
        Mu.append(mu)
        Mu.append(mu**2)
        Nu.append(nu)
        Nu.append(nu**2)
    for i in range(num_samples//2):
        mu = grf.gaussian_random_field(alpha =10,size=length, flag_normalize=False).to(device)
        nu = grf.gaussian_random_field(alpha =10,size=length, flag_normalize=False).to(device)
        Mu.append(mu)
        Mu.append(mu**2)
        Nu.append(nu)
        Nu.append(nu**2)
    ...
    if incl_random_shapes:
        mu = rand_shapes(num_samples // 2, length**2, dust_const, pairs=False).float().reshape(-1,length, length).to(device)
        nu = rand_shapes(num_samples // 2, length**2, dust_const, pairs=False).float().reshape(-1,length, length).to(device)
        for x in mu:
            mu_grf = grf.gaussian_random_field(alpha = 5,size=length, flag_normalize=False).to(device)
            Mu.append(x)
            Mu.append(x*mu_grf)
        for x in nu:
            nu_grf = grf.gaussian_random_field(alpha = 5,size=length, flag_normalize=False).to(device)
            Nu.append(x)
            Nu.append(x*nu_grf)
