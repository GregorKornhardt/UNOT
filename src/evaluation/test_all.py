...
def set_evaluation(
    ...
    with torch.no_grad():
        ...
        _,_,distance_1000_step = sink.sink_vec_dist(set_mu, set_nu, cost_matrix, epsilon, ones,1000)
        _,_,distance_ones = sink.sink_vec_dist(set_mu, set_nu, cost_matrix, epsilon, ones,1)
        _,_,distance_predicted = sink.sink_vec_dist(set_mu, set_nu, cost_matrix, epsilon, v_0,1)
        _,_,distance_gauss = sink_vec_dist_gauss(set_mu, set_nu, cost_matrix, epsilon, u0_gauss,1)

        time_predicted = sinkhorn_computation_predictor(set_mu, set_nu, cost_matrix, epsilon, distance_1000_step, predictor, max_time_s=max_time_s)
        time_ones = sinkhorn_computation_time(set_mu, set_nu, cost_matrix, epsilon, distance_1000_step, max_time_s=max_time_s)
        time_gauss = sinkhorn_computation_time_gauss(set_mu, set_nu, cost_matrix, epsilon, distance_1000_step, u0_gauss, max_time_s=max_time_s)

        k_ones = sinkhorn_iteration_until_error(set_mu, set_nu, cost_matrix, 0.01, ones, 1000, distance_1000_step, error_bound)
        k_predicted = sinkhorn_iteration_until_error(set_mu, set_nu, cost_matrix, 0.01, v_0, 1000, distance_1000_step, error_bound)
        k_gauss = sinkhorn_iteration_until_error(set_mu, set_nu, cost_matrix, 0.01, u0_gauss, 1000, distance_1000_step, error_bound)

        violation_ones = sinkhorn_constraint_violation(set_mu, set_nu, cost_matrix, epsilon, ones)
        violation_predicted = sinkhorn_constraint_violation(set_mu, set_nu, cost_matrix, epsilon, v_0)
        violation_gauss = sinkhorn_constraint_violation_gauss(set_mu, set_nu, cost_matrix, epsilon, u0_gauss)

        relativ_error_iter_ones = sinkhorn_error_iteration(set_mu, set_nu, cost_matrix, epsilon, ones, 50, distance_1000_step)
        relativ_error_iter_predicted = sinkhorn_error_iteration(set_mu, set_nu, cost_matrix, epsilon, v_0, 50, distance_1000_step)
        relativ_error_iter_gauss = sinkhorn_error_iteration_gauss(set_mu, set_nu, cost_matrix, epsilon, u0_gauss, 50, distance_1000_step)

        # calulate relative error and append to dictionary
        dict_relative_error['predicted'] = torch.abs(distance_predicted - distance_1000_step)/distance_1000_step
        dict_relative_error['ones'] = torch.abs(distance_ones-distance_1000_step)/distance_1000_step
        dict_relative_error['gauss'] = torch.abs(distance_gauss-distance_1000_step)/distance_1000_step
        ...
        del v_0, distance_1000_step, distance_ones, distance_predicted, time_predicted, time_ones, k_ones, k_predicted, u0_gauss, distance_gauss, time_gauss, k_gauss, violation_ones, violation_predicted, violation_gauss


    return dict_relative_error, dict_time, dict_iteration, set_violation, dict_relative_error_iter, dict_violation_iter, relative_error_dim
...
def relative_error_over_dimension(
    ...
    for length in tqdm(range(min_len, max_len, 2)):
        ...
        set_mu = df.preprocessor(original_set_mu.reshape(-1, original_len, original_len), length, 1e-6).to(original_set_mu.device)
        set_nu = df.preprocessor(original_set_nu.reshape(-1, original_len, original_len), length, 1e-6).to(original_set_nu.device)
        ...
        with torch.no_grad():
            ...
            dict_relative_error['predicted'].append((torch.abs(distance_predicted - distance_1000_step)/distance_1000_step).cpu())
            dict_relative_error['ones'].append((torch.abs(distance_ones-distance_1000_step)/distance_1000_step).cpu())
            dict_relative_error['gauss'].append((torch.abs(distance_gauss-distance_1000_step)/distance_1000_step).cpu())

    del v_0, distance_1000_step, distance_ones, distance_predicted, distance_gauss, u0_gauss, ones
...
def set_ot_distance(
    ...
    for i in tqdm(range(number_samples)):
        with torch.no_grad():
            ...
            # calulate relative error and append to dictionary
            dict_relative_error['predicted'].append(torch.abs(distance_predicted - distance_1000_step)/distance_1000_step)
            dict_relative_error['ones'].append(torch.abs(distance_ones-distance_1000_step)/distance_1000_step)
...
def sinkhorn_error_iteration(
    ...
    for _ in range(maxiter):
        ...
        for i in range(set_mu.shape[0]):
            ...
            v = V[i]
            G = torch.diag(u) @ K @ torch.diag(v)
            mcv[i] = torch.norm(torch.sum(G, axis=0) - set_mu[i], p=1) + torch.norm(torch.sum(G, axis=1) - set_nu[i], p=1) / 2
...
def set_iteration_until_given_error(
    ...
    for i in tqdm(range(number_samples)):
        with torch.no_grad():
            ...
            _, _, _, distance_1000_step = sink.sink(set_mu, set_nu, cost_matrix, 0.01, ones, 1000)
...
def sinkhorn_constraint_violation(
    ...
    for i in range(set_mu.shape[0]):
        ...
        v = nu / (K.T @ u)
        G = torch.diag(u) @ K @ torch.diag(v)
        costraint_violation = torch.norm(torch.sum(G, axis=1) - mu, p=1) + torch.norm(torch.sum(G, axis=0) - nu, p=1) 
...
def set_computation_time(
    ...
    for i in tqdm(range(number_samples)):
        with torch.no_grad():            
            _,_,_,distance_1000_step = sink.sink(set_mu, set_nu, cost_matrix, epsilon, ones,1000)
            
            v0_pred = torch.exp(predictor(set_mu[i : i+1], set_nu[i : i+1])).flatten()
            
            mu_jnp = jnp.array(set_mu[i].cpu().numpy())
            nu_jnp = jnp.array(set_nu[i].cpu().numpy())
            u0_gauss = get_gauss_init(geom, mu_jnp, nu_jnp)
            
            time_predicted = sinkhorn_computation_time(set_mu, set_nu, cost_matrix, epsilon, distance_1000_step, v0_pred, max_time_s)
            time_ones = sinkhorn_computation_time(set_mu, set_nu, cost_matrix, epsilon, distance_1000_step, ones, max_time_s)
            time_gauss = sinkhorn_computation_time(set_mu, set_nu, cost_matrix, epsilon, distance_1000_step, u0_gauss, max_time_s)
