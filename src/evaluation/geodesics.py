...
def mccann_interpolation_bilinear(
        P, 
        grid_points, 
        t, 
        N
...
def mccann_network(
    ...
    for i in range(0, 5):
        t = i / 4
        gamma_t = mccann_interpolation_bilinear(G.cpu().detach().numpy(), X.cpu().detach().numpy(), t, 28)
        plan_geodesic.append(gamma_t)
    
    return plan_geodesic


def barycenter_geodesic(
        predictor, 
        mu, 
        nu, 
        cost_matrix, 
    ...
    for weights in weights_list:
        gamma_t = barycenter(predictor, bar, cost_matrix.to(device), torch.tensor(weights).to(device), nits=200)
