...
def plot_approximation(set_approx, model_name):
    data_dict = {}
    ...
    plt.yticks(torch.arange(0.0, 0.95, 0.05))
    ...
    for i, group in enumerate(group_labels, start=1):
        plt.text(
            i, -0.15, group, ha="center", va="top", fontsize=9, color="gray"
        )  # Position unter den Labels
...
def plot_marginal_constraint(set_marginal, model_name):
    ...
    for i, group in enumerate(group_labels, start=1):
        plt.text(
            i, -0.12, group, ha="center", va="top", fontsize=9, color="gray"
        )  # Position unter den Labels
...
def print_iteration(
    ...
    print(
        f"Predicted: {torch.mean(torch.tensor(set_iter_1['predicted'], dtype=torch.float32)).item()} +/- {torch.std(torch.tensor(set_iter_1['predicted'], dtype=torch.float32)).item()}"
    )
    print(
        f"Ones: {torch.mean(torch.tensor(set_iter_1['ones'], dtype=torch.float32)).item()} +/- {torch.std(torch.tensor(set_iter_1['ones'], dtype=torch.float32)).item()}"
    )
    print(
        f"Predicted: {torch.mean(torch.tensor(set_iter_2['predicted'], dtype=torch.float32)).item()} +/- {torch.std(torch.tensor(set_iter_2['predicted'], dtype=torch.float32)).item()}"
    )
    print(
        f"Ones: {torch.mean(torch.tensor(set_iter_2['ones'], dtype=torch.float32)).item()} +/- {torch.std(torch.tensor(set_iter_2['ones'], dtype=torch.float32)).item()}"
    )
    print(
        f"Predicted: {torch.mean(torch.tensor(set_iter_3['predicted'], dtype=torch.float32)).item()} +/- {torch.std(torch.tensor(set_iter_3['predicted'], dtype=torch.float32)).item()}"
    )
    print(
        f"Ones: {torch.mean(torch.tensor(set_iter_3['ones'], dtype=torch.float32)).item()} +/- {torch.std(torch.tensor(set_iter_3['ones'], dtype=torch.float32)).item()}"
    )
...
def plot_time(set_time, model_name):

    def distance_over_time(set_time):
    ...
    for i, key in enumerate(set_time.keys()):
        time_bucket_pred_1, distance_pred_1 = distance_over_time(
            set_time[key]["predicted"]
        )
        time_bucket_ones_1, distance_ones_1 = distance_over_time(set_time[key]["ones"])
        time_bucket_gauss_1, distance_gauss_1 = distance_over_time(
            set_time[key]["gauss"]
        )
        # Plot the distance over time

        l1 = axs[i].plot(
            time_bucket_pred_1, distance_pred_1, label="UNOT", color="crimson"
        )
        l3 = axs[i].plot(
            time_bucket_ones_1, distance_ones_1, label="Ones", color="indigo"
        )
        l2 = axs[i].plot(
            time_bucket_gauss_1, distance_gauss_1, label="Gauss", color="cornflowerblue"
        )
...
def plot_error_dim_eps_matrix(data_set, set_error_dim, model_name):
    fig, axes = plt.subplots(
        len(data_set), len(data_set), figsize=(17, 15), sharex=True, sharey=True
    )
    ...
    for i, data1 in enumerate(data_set):
        for j, data2 in enumerate(data_set):
            ...
            if j <= i:
                ...
                ax.set_xticklabels(
                    [r"$10^{-2}$", r"$10^{-1}$", r"$10^{0}$"], fontsize=16
                )
                ...
                for key in set_error_dim[data1][data2].keys():
                    ...
                    sns.lineplot(
                        ...
                        label=(
                            "UNOT"
                            if key == "predicted"
                            else ("Ones" if key == "ones" else "Gauss")
                        ),
                    )
                    ...
                    ax.set_yticklabels(
                        [f"{x:.1f}" for x in np.arange(0, 0.9, 0.2)], fontsize=16
                    )
...
def main():
    ...
    parser.add_argument(
        "--dimension", type=int, default=28, help="Dimension of the measures."
    )
    parser.add_argument(
