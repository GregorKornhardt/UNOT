...
def load_mlp(
    ...
    predictor = Predictor(dimension**2, int(width_predictor * dimension**2), num_layers).to(device)
...
def apply_fno(predictor, mu, nu, grid=False):
    ...
    if grid:
        ...
        input = torch.cat((mu.reshape(-1, 1, length, length),
                            nu.reshape(-1, 1, length, length),
                            grid.repeat(mu.shape[0], 1, 1, 1).to(nu.device)), 1)
    else:
        input = torch.cat((mu.reshape(-1, 1, length, length), 
                           nu.reshape(-1, 1, length, length)), 1)
...
def apply_predictor(predictor, mu, nu, eps, grid):
    ...
    if grid:
        ...
        input = torch.cat((mu.reshape(-1, 1, length, length), 
                           ...
                           eps * torch.ones_like(mu).reshape(-1, 1, length, length)), 1)
    else: 
        input = torch.cat((mu.reshape(-1, 1, length, length), 
                           nu.reshape(-1, 1, length, length), 
                           eps * torch.ones_like(mu).reshape(-1, 1, length, length)), 1)
