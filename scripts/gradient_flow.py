...
class SD_outer(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        g = torch.einsum("ij,ijn->in", G, V - U_x) - torch.einsum(
            "ij,ijn->in", G_x, V_x - U_x_x
        )
...
def gradient_flow_image(predictor, y, niter=100):
    ...
    cost_matrix = cost.fast_get_cost_matrix(math.isqrt(y.shape[1]), device=y.device).to(
        y.device
    )
...
def plot_all(images, ground_truth=None):
    ...
    for k, batch in enumerate(images):
        if k % 10 == 0:
            for i in range(batch.shape[0]):
                ...
                small_image = (
                    (batch[i].reshape(n, n) - batch[i].min())
                    / (batch[i].max() - batch[i].min())
                    * 255
                )
                # Place the reshaped image into the large image
                large_image[y_offset : y_offset + n, x_offset : x_offset + n] = (
                    small_image
                )
    ...
    for i in range(ground_truth.shape[0]):
        ...
        small_image = (
            (ground_truth[index].reshape(n, n) - ground_truth[index].min())
            / (ground_truth[index].max() - ground_truth[index].min())
            * 255
        )
        # Place the reshaped image into the large image
