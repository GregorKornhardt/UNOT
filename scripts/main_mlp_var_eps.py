...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Number of random epsilon per batch, for given x it will be 2^x.",
    )
...
def main():
    ...
    try:
        if args.length == 28:
            ...
            test_data01 = torch.load(
                f"{dir_prefix}/Data/test_set__dim_64__eps_0.01.pt", weights_only=True
            )
            test_data05 = torch.load(
                f"{dir_prefix}/Data/test_set__dim_64_600_eps_0.05.pt", weights_only=True
            )
            test_data1 = torch.load(
                f"{dir_prefix}/Data/test_set__dim_64_600_eps_0.1.pt", weights_only=True
            )
    ...
    torch.save(
        ...
        f"{dir_prefix}/Models/predictor_{args.length}_{run_name}_.pt",
    )
