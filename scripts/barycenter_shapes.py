...
def shape_measures(dimension, device):
    ...
    mu = torch.stack(
        [
            matrix_heart_tensor.flatten(),
            donut(),
            matrix_circles_tensor.flatten(),
            matrix_x_tensor.flatten(),
        ]
    ).to(device)
...
def plot_barycenter(predictor, device):
    ...
    plt.savefig(
        "Images/color_image_barycenter.pdf", bbox_inches="tight", pad_inches=0.1
    )
