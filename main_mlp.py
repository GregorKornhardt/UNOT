...
def parse_args():
    ...
    return  parser.parse_args()  # Convert args to dictionary
...
def main():
    ...
    predictor = Predictor(dimension, dim_predictor, num_layers=args.num_layers).to(device)
    ...
    # save model
    torch.save(trainer.predictor.state_dict(), f"{dir_prefix}/Models/predictor_{args.length}_{args.name}_{run_name}.pt")
